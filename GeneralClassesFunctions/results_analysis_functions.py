...
def results_analysis(directory):
    ...
    Return:
        ...
        leakage_timeseries  Array of leakage in each LDAR program in each realization at each time step
    """
    files = [f for f in listdir(directory) if isfile(join(directory, f))]
    n_realizations = len(files)
    sample = load(open(directory + '/' + files[0], 'rb'))
    n_techs = len(sample.tech_dict)
    # Initialize an array to store a time series of leakage for every LDAR program in every realization
...
def npv_calculator(filepath):
    ...
    for index in range(0, len(techs)):
        ...
        gas_value_no_repair[index] = sum((np.array(sample.no_repair_leakage)-sample.tech_dict[techs[index]].leakage) /
        ...
        if techs[index] != 'Null':
            ...
            gas_value_n[ind] = sum((null_leakage-sample.tech_dict[techs[index]].leakage) / discount_array) * \
